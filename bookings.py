# ==============================
# ULTRA FAST MULTI PROPERTY BOOKING MODE
# DATE-WISE WITH CHARTS + RANKING
# ==============================
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            for b in bookings.values():
                ...
                try:
                    ci_date = datetime.strptime(checkin, "%Y-%m-%d").date()
                except:
                    continue
                ...
                if src not in date_map[ci_date]:
                    src = "OBA"

                rooms = int(b.get("no_of_rooms", 1) or 1)

                date_map[ci_date][src] += rooms
...
async def main():
    ...
    for _ in range(MAX_FULL_RUN_RETRIES):
        ...
        for key,(P,res) in zip(
            list(pending.keys()),
            zip(pending.values(), results)
        ):
    ...
    def create_sheet(ws, date_map):
        ...
        footer_row = start_chart_row + 9*chart_gap
