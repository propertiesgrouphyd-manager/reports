    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            for b in bookings.values():
                ...
                mapping.append((ci, co, rooms))

            results = await asyncio.gather(*tasks, return_exceptions=True)

            for res, (ci, co, rooms) in zip(results, mapping):
                ...
                while stay_day < co:
                    ...
                    if tf_dt <= stay_day <= tt_dt:
                        ...
                        row["discount"] += disc_d
                        row["balance"] += bal_d
                        row["total"] += (cash_d + qr_d + online_d + disc_d + bal_d)
                        row["urns"] += rooms
...
async def main():
    ...
    def create_sheet(ws, date_map):
        ...
        ws.append(["Date","Cash","QR","Online","Discount","Balance","Total","URNs"])
        ...
        sum_cash=sum_qr=sum_online=sum_discount=sum_balance=sum_total=sum_urns=0
        ...
        for idx,d in enumerate(date_list):
            ...
            ws.append([
                d.strftime("%d-%m-%Y"),
                cash,qr,online,discount,balance,total,urns
            ])
