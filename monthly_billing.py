...
def generate_monthly_excel(records):
    start_dt, end_dt = get_previous_month_range()
    ...
    ws["A1"] = "MONTHLY PROPERTY REPORTS BILL"
    ...
    ws["A3"] = f"Billing Month : {month_name}"
    ...
    ws.cell(row=row, column=2).value = days_in_month
...
async def send_telegram_excel(filename):
    ...
    async with aiohttp.ClientSession(
        ...
        with open(filename, "rb") as f:
            ...
            form.add_field(
                "caption",
                f"📊 Monthly Property Reports Bill\nMonth: {start_dt.strftime('%B %Y')}"
            )
...
async def main():
    ...
    filename = generate_monthly_excel(
        records
    )
